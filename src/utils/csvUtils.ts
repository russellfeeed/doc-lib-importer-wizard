  ...
  documents
    ...
    if (isCircularLetter) {
      ...
      row = {
        ...
        'File URL': escapeCsvValue(docFile.fileUrl || `https://dev.members.nsi.org.uk/wp-content/uploads/${getCurrentUploadPath()}/${docFile.name.includes('.') ? docFile.name : docFile.name + '.pdf'}`),
        'Direct URL': escapeCsvValue(docFile.directUrl || `https://dev.members.nsi.org.uk/wp-content/uploads/${getCurrentUploadPath()}/${docFile.name.includes('.') ? docFile.name : docFile.name + '.pdf'}`),
